...
} from "matrix-react-sdk/src/utils/AutoDiscoveryUtils";
import classNames from "classnames";
// import * as Lifecycle from 'matrix-react-sdk/src/Lifecycle';
import { MatrixClientPeg } from "matrix-react-sdk/src/MatrixClientPeg";
import AuthPage from "../../views/auth/SamvaadAuthPage";
import Login from "matrix-react-sdk/src/Login";
import dis from "matrix-react-sdk/src/dispatcher/dispatcher";
import AuthHeader from "../../views/auth/SamvaadAuthHeader";
// Phases
...
export default createReactClass({
    ...
    _replaceClient: async function (serverConfig) {
        ...
        try {
            ...
            console.log(
                "Unable to determine is server needs id_server param",
                e
            );
        }
        ...
        try {
            ...
            if (e.httpStatus === 401) {
                ...
                try {
                    ...
                    const hasSsoFlow = flows.find(
                        (f) =>
                            f.type === "m.login.sso" || f.type === "m.login.cas"
                    );
                    if (hasSsoFlow) {
                    ...
                    console.error(
                        "Failed to get login flows to check for SSO support",
                        e
                    );
                    showGenericError(e);
                }
    ...
    _onUIAuthFinished: async function (success, response, extra) {
        ...
        const sessionOwner = require("matrix-react-sdk/src/Lifecycle").getStoredSessionOwner();
        const sessionIsGuest = require("matrix-react-sdk/src/Lifecycle").getStoredSessionIsGuest();
        if (
            sessionOwner &&
            !sessionIsGuest &&
    ...
    _onLoginClickWithCheck: async function (ev) {
        ...
        const sessionLoaded = await require("matrix-react-sdk/src/Lifecycle").loadSession(
            { ignoreGuest: true }
        );
        if (!sessionLoaded) {
